...
function render(arr, page = 0) {
    ...
    for (let i = minElement; i < maxElement; i++) {
        container.insertAdjacentHTML('beforeend',`
            <div>
            <h3 class = "curse"><i>${arr[i].name}</i></h3>
    ...
    btn.forEach((e,i)=>e.addEventListener('click', ()=>location.href=`http://127.0.0.1:5500/pages.html?id=${i}`))
}
//4
